...
def runDWT():
    ...
    np.save("C:/Users/GUDDANTI MEGHANA/OneDrive/Desktop/model/"+name,watermarkedImage)
    np.save("C:/Users/GUDDANTI MEGHANA/OneDrive/Desktop/model/CA_"+name,cA)
...
def runExtraction():
    ...
    # "C:\Users\GUDDANTI MEGHANA\OneDrive\Desktop\C4-Implementation of Secured Watermarking Mechanism Using Cryptography and Bit Pair Matching\model\1.jpg.npy"
    # "C:\Users\GUDDANTI MEGHANA\OneDrive\Desktop\model\1.jpg.npy"
    # img = np.load("/model/"+wm+".npy")
    # cA = np.load("/model/CA_"+wm+".npy")
    img = np.load("C:/Users/GUDDANTI MEGHANA/OneDrive/Desktop/model/"+wm+".npy")
    cA = np.load("C:/Users/GUDDANTI MEGHANA/OneDrive/Desktop/model/CA_"+wm+".npy")
...
def runSVD():
    ...
    # SVD of cover image
    ucvr, wcvr, vtcvr = np.linalg.svd(coverImage, full_matrices=True, compute_uv=True)
    Wcvr = np.zeros((m, n), np.double)
    ...
    for i in range(x):
        for j in range(y):
            Wcvr[i, j] = (Wcvr[i, j] + 0.01 * watermarkImage[i, j])

    # SVD of Wcvr
    u, w, v = np.linalg.svd(Wcvr, full_matrices=True, compute_uv=True)

    # Watermarked Image
    S = np.zeros((m, n), np.double)
    S[:m, :n] = np.diag(w)
    ...
    text.insert(END, "SVD PSNR : " + str(psnr) + "\n")
    text.insert(END, "SVD MSE  : " + str(mse) + "\n")
    text.insert(END, "SVD SSIM : " + str(ssim_value) + "\n\n")
    text.update_idletasks()
